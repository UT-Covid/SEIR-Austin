...
source("code/models/mobility+agegroups+ar1+mu+pres+risk-simple+Vacc+delta.R.cpp")
source("code/common/small_func.R")
source("code/common/prepare_scenario.R")


days_average = 1
#days_average = 7

num_forecast_days = 180
...
#   args=c("short", "data/CV19AdmitDisch_UT_20210723.xlsx","data/KeyIndicatorsUpdate_20210723.xlsx","newinit1")
 #   args=c("history", "data/CV19AdmitDisch_UT_20210702.xlsx","30" ) 
 args=c("continue","after.smooth.20210728school1a.Rda","scen.file:code/parameters_for_run_school.R","scen:3","no:pdf","no:dash","no:save.sim","no:save.smooth")
 #  args=c("continue","after.smooth.20210723school2.Rda")
}
...
if( short.run ) {
    ...
    #    file="before.plot.11.23.2020B.Rds", 
        file="init.states.after.analysis.20210319.Rds",
    #     date0=ymd("2020-10-01") 
        date0=ymd("2021-03-01") 
    )
}
...
if( "scen.file" %in% names(arg.list) ) {
    scen.file = arg.list["scen.file"]
} else {
    scen.file = "code/parameters_for_run.R"
}

if( "scen" %in% names(arg.list) ) {
    scen.index = as.numeric( arg.list["scen"] )
} else {
    scen.index = 1
}
...
par.list = parameters.for.run( par.list.compact )$pars[[ scen.index ]]
par.list$name = parameters.for.run( par.list.compact )$names[[ scen.index ]]

cat("Read parameters from", scen.file,"\n")
cat("Giving this list:\n")
cat(parameters.for.run( par.list.compact )$names, sep="\n")
...
if( ( gsub(pat="after.",rep="",analysis.continue.after) %in% analysis.steps)  )  {
    ...
    # If scenario was given in arguments, save it
    if( "scen" %in% names(arg.list) | "scen.file" %in% names(arg.list) | "name.add" %in% names(arg.list)) {
        save.vars$par.list=par.list
        save.vars$par.index=par.index
    }
    ...
    if( "par.list" %in% names(save.vars) ) {
        run.name = paste0( run.name, ".", par.list$name)
    }
...
#    covars=covars[order(covars$date),]
    ...
    covars = covars %>%
        right_join( get.scenario( covars$date, par=par.list ), by="date" ) %>%
        mutate( dZ = 0)  # @audit covars dZ set to 0 here - only future dZ sims

    if( !"dZ" %in% colnames(covars)) {
        covars = covars %>% mutate( dZ=0) 
    }
...
#    l = foreach( i=ii , .packages = c("magrittr", "pomp") ) %dopar% {  # @audit change back to dopar
...
#  ) %dopar% { @audit change back to dopar
  ) %dopar% {
    mf2 <- covid_mod %>%
        mif2(
            Np= N.particles,
...
#                 }
  ...
  m
    ...
    if( exists("init.states") ) {
        # Start from previously saved states
        ...
        i.old.states = covid_statenames %in% dimnames(init_res$smoothed_states)[[1]]
        not.in.smoothed_states = matrix( 0, sum( !i.old.states ), dim(init_res$smoothed_states)[3] )
        rownames( not.in.smoothed_states) = covid_statenames[!i.old.states]
        smoothed_states_t0 = rbind( init_res$smoothed_states[ covid_statenames[i.old.states], init.states$t0, ], not.in.smoothed_states )
        rm( "init_res")
...
#########################
  ...
  #  filtered_states = filtered_states_unsampled[,,sample_ind]
    ...
    if( !("save.smooth" %in%  arg.list[ names(arg.list)=="no"] ) ) {
                    save.session(file=paste0("after.smooth.",run.name,".Rda"))
        cat("done saving\n")
    }
...
#    filtered_states = filtered_states_unsampled[,,sample_ind]
    ...
    covars$dZ = as.numeric( covars$dZ )
    # @todo new_covars
    new_covars = covars %>% 
...
#        mutate( dZ = 0) %>%    # should be set in covars, but just in case it wasn't
    ...
    x = get.scenario( new_covars$date, par=par.list )
    xn = colnames( x)
    xn = xn[xn != "date" ]

new_covars = new_covars %>%
        select( -xn ) %>%
        right_join( x, by="date" ) 

    
    if( !"dZ" %in% colnames(new_covars)) {
        new_covars = new_covars %>% mutate( dZ=0) 
    }
...
if( do.this.step( analysis.continue.after, current.step="analysis") )  { 
    ...
    if( exists("init.states") ) {
        ...
        i.old.states = covid_statenames %in% dimnames( old$smoothed_states)[[1]]
        not.in.smoothed_states = array( 0, c( sum( !i.old.states ), dim( old$smoothed_states)[2:3]) )
        dimnames( not.in.smoothed_states)[[1]] = covid_statenames[!i.old.states]
        smoothed_states_new = abind( old$smoothed_states, not.in.smoothed_states, along=1 )
        ...
        empty_old = array( 0, c( dim(smoothed_states)[1], sum(i), j  ))
        dimnames( empty_old)[[1]] = dimnames( smoothed_states)[[1]]
        dimnames( empty_old)[[2]] = dimnames( old$smoothed_states)[[2]][i]
        common = intersect( dimnames(old$smoothed_states)[[1]], dimnames(smoothed_states)[[1]])                   
        empty_old[ common,, ] = old$smoothed_states[common,i,1:j]
        smoothed_states = abind( empty_old, smoothed_states[,,1:j], along=2)
    ...
    rolling = function( a, dims=c(2), days=7 ) {
        if( is.null(dim(a))) {
            rollmean( a, k=days, na.pad=T)
        } else {
            apply(a, dims, function(x) {
                rollmean(x, k=days, na.pad=T)
            })
        }
    }
   ...
   source("code/common/NGM_R_with_S_and_V.R")
    ...
    myfun3 = function(beta, epsilon_V, S_prop, V_prop) get_rtV(
        beta=beta, phi=(phi), pop=pop, pop_ratio_mat=pop_ratio_mat, sigma=sigma, tau=tau, rho_A=rho_A, rho_Y=rho_Y, 
                                        gamma_A=gamma_A, gamma_Y=gamma_Y, omega_A=omega_A , omega_Y=omega_Y , omega_P=omega_P , S_prop, 
                                        epsilon_V=epsilon_V, tau_V=tau_V, V_prop=V_prop )
    ...
    if( calc.r0) {
        ...
        V.states = matrix( paste( "V",matrix(1:2,2,5), matrix(1:5,2,5,byrow = T), sep="_"), 2, 5)
if(T) {
     rt = foreach(i=1:dim(smoothed_states)[3], .combine = cbind, .packages = c("magrittr")) %dopar% 
     {
            ...
            if(sum(is.na(Age.S.prop))>0 | sum(is.infinite( Age.S.prop))>0) { 
                na.i <<- c(na.i,i) 
            }
    ...
    dimnames(rt) = dimnames( smoothed_states)[2:3]
     ...
     rtV = foreach(i=1:dim(smoothed_states)[3], .combine = cbind, .packages = c("magrittr")) %dopar% 
     {
            Age.Pop.N  = sapply( 1:5,function(g) smoothed_states[ all.states[grep(paste0("_",g,"$"),all.states)]  ,,i] %>% colSums ) 
            Age.S.N    = sapply( 1:5,function(g) smoothed_states[ S.states[,g]  ,,i] %>% colSums ) 
            Age.S.prop = Age.S.N / Age.Pop.N
            Age.V.N    = sapply( 1:5,function(g) smoothed_states[ V.states[,g]  ,,i] %>% colSums ) 
            Age.V.prop = Age.V.N / Age.Pop.N
            if(sum(is.na(Age.S.prop))>0 | sum(is.infinite( Age.S.prop))>0) { 
                na.i <<- c(na.i,i) 
            }
            epsilon_V = new_covars$variant_p * var_incr_V_infection  / (1.0+ new_covars$variant_p * (var_beta_mult - 1.0 ) ) 
            rt_i = sapply( seq_len(nrow(Age.S.N)), function(j) 
                        myfun3( beta=smoothed_states["Beta",j , i], epsilon_V = epsilon_V[j], S_prop=Age.S.prop[j,], V_prop=Age.V.prop[j,] )   )
            
    }
    dimnames(rtV) = dimnames( smoothed_states)[2:3]
}

    get_rt <- function(case_vec){
        case_vec <- ifelse(case_vec < 0, 0, case_vec)
        ## Disease parameters
        shape=3.63
        rate = 0.71
        mean = shape/rate
        std = (shape/rate^2)^(0.5)
        res_with_imports <- estimate_R(case_vec, 
                                    method = "parametric_si", 
                                    config = make_config(list( mean_si = mean, 
                                                                std_si = std))) %>% 
        suppressMessages()
        rt_vec <- c(rep(NA, length(case_vec) - length(res_with_imports$R$`Median(R)`)), 
            res_with_imports$R$`Median(R)`)
        if(length(case_vec) != length(rt_vec)){
        browser()
        }
        # print(rt_vec)
        rt_vec
    ...
    rtEmp <- smoothed_states['NI',,] %>% 
                apply(2, FUN = function(x){c(0,diff(x))}) %>% 
                apply(2, FUN = get_rt)
    ...
    rtV   = array( rtV,   dim = c(1,dim(smoothed_states)[2:3]))
    rtEmp = array( rtEmp, dim = c(1,dim(smoothed_states)[2:3]))
    dimnames(r0) =c(list("R0"),  dimnames(smoothed_states)[2:3])
    dimnames(rt) =c(list("RtOld"),  dimnames(smoothed_states)[2:3])
    dimnames(rtV)=c(list("RtV"), dimnames(smoothed_states)[2:3])
    dimnames(rtEmp) =c(list("Rt"),  dimnames(smoothed_states)[2:3])
    smoothed_states =abind( smoothed_states, r0, rt, rtV, rtEmp, along=1)
    ...
    rtCI = cbind( rtCI, fracIncr=rowMeans(rtEmp>1)*100)

    rtOldCI = getCI( smoothed_states['RtOld', ,], alpha=alpha)
    rtOldCI = cbind( rtOldCI, fracIncr=rowMeans(rt>1)*100)

    rtVCI = getCI( smoothed_states['RtV', ,], alpha=alpha)
    rtVCI = cbind( rtVCI, fracIncr=rowMeans(rtV>1)*100)
    ...
    EVCI = getCI(  smoothed_states['EV_1_1',,] + smoothed_states['EV_2_1',,] +
                    smoothed_states['EV_1_2',,] + smoothed_states['EV_2_2',,] +
                    smoothed_states['EV_1_3',,] + smoothed_states['EV_2_3',,] +
                    smoothed_states['EV_1_4',,] + smoothed_states['EV_2_4',,] +
                    smoothed_states['EV_1_5',,] + smoothed_states['EV_2_5',,]  , alpha=alpha)
