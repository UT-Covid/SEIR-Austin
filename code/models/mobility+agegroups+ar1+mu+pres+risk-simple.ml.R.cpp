...
//      leaving_E[g][i] = (E[g][i] > 0) ? rbinom(E[g][i], 1.0 - exp(- sigma * dt)) : 0.0; // end of encubation period
  ...
  for (int g=0; g<2; g++) {
    for (int i=0; i < 5; i++) {
//      new_PA[g][i] = ( leaving_E[g][i] > 0) ? rbinom(leaving_E[g][i], 1.0 - tau              ) : 0.0;
//      new_IA[g][i] = ( PA[g][i]        > 0) ? rbinom(PA[g][i]       , 1.0 - exp(- rho_A * dt)) : 0.0;
//      new_PY[g][i] = leaving_E[g][i] - new_PA[g][i];
//      new_IY[g][i] = ( PY[g][i]        > 0) ? rbinom(PY[g][i]       , 1.0 - exp(- rho_Y * dt)) : 0.0;
      new_PA[g][i] = BIN(leaving_E[g][i], 1.0 - tau );
...
//      leaving_IY[g][i] = (IY[g][i] > 0) ? rbinom(IY[g][i], 1.0 - exp(- leaving_IY_rate * dt)) : 0.0;
//      new_H[g][i] = (leaving_IY[g][i] > 0) ? rbinom(leaving_IY[g][i], pi * eta / leaving_IY_rate ) : 0.0;
...
//      leaving_H[g][i] = (H[g][i] > 0) ? rbinom(H[g][i], 1.0 - exp(- leaving_H_rate * dt)) : 0.0;
//      recovering_H[g][i] = (leaving_H[g][i] > 0) ? rbinom(leaving_H[g][i], (1 - nu) * gamma_H / leaving_H_rate) : 0.0;
